...
export async function generateMetadata(): Promise<Metadata> {
  ...
  return {
    title: config["SITE_TITLE"] || "Store Pickaxe",
    description: config["SITE_DESCRIPTION"] || "모바일 게임 스토어 리뷰 분석",
  };
}

export default async function RootLayout({ children }: { children: React.ReactNode }) {
  const initialTexts = await getUITexts();

  return (
    <html lang="ko">
      <body style={{ background: "#FAFAFA" }}>
        <TextsProvider initial={initialTexts}>
          <nav
            style={{
              position: "sticky",
              top: 0,
